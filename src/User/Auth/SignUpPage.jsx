  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      ...
      if (validationErrors.wallet) {
        toast.error(validationErrors.wallet);
      }
      return;
    }
    ...
    setIsLoading(true);
    try {
      const referralCode = formData.referral.trim() || appConfig.defaultReferral;
      const response = await axios.post(`${appConfig.baseURL}/user/auth/signup`, {
        email: formData.email,
        password: formData.password,
        referredBy: referralCode,
        walletAddress: walletAddress, // Include wallet address in the request
      });
      ...
      setErrors((prev) => ({
        ...
        wallet: errorMessage.includes("wallet") ? errorMessage : prev.wallet,
      }));
    } finally {
      setIsLoading(false);
    }
            ...
            {isConnected && (
              <div>
                <input
                  ...
                  className="w-full px-4 py-3 rounded-md bg-secondary/10 border border-white/10 focus:outline-none"
                  disabled
